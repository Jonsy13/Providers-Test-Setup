...
describe("Testing the create Workflow Utility", () => {
  ...
  it("Checking the accessibility of the choose-cluster page", () => {
    ...
    cy.get("[data-cy=AgentsRadioGroup]").should("be.visible");
  });

  it("Checking choose cluster screen functionality", () => {
    cy.get('[type="radio"]').eq(0).check(); //Selecting Self Cluster from DropDown
    cy.get("[data-cy=ControlButtons]").children().eq(0).should("be.enabled"); //Only Next Button is there on this Screen
    cy.get("[data-cy=ControlButtons]").children().eq(0).click();
    cy.url().should("contain", "/create-workflow");
  });

  it("Checking Choose Workflow screen", () => {
    cy.get("[data-cy=WorkflowsRadioGroup]").should("be.visible");
    cy.get("[data-cy=WorkflowsRadioGroup]").children().eq(0).check();
